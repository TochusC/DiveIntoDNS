 ...
 * @File    :   cname_nodnssec.go
 * @Contact :	tochus@163.com
 * @License :   (C)Copyright 2024
 * @Description: A DNS server that responds to DNS queries with CNAME Chain without DNSSEC.
 *
 ...
 * 18/10/24 18:35      4stra       1.0.1	   Optimizations
 * 18/10/24 18:58      4stra       1.1.0	   CNAME NODNSSEC TEST
 */
...
var answerSection = map[string][]layers.DNSResourceRecord{
	...
	"cname.nodnssec.test": {
		{
			Name:  []byte("cname.nodnssec.test"),
			Type:  layers.DNSTypeCNAME,
			Class: layers.DNSClassIN,
			TTL:   uint32(globalTTLC),
			CNAME: []byte("cname0.nodnssec.test"),
		},
	},
	"cname0.nodnssec.test": {
		{
			Name:  []byte("cname0.nodnssec.test"),
			Type:  layers.DNSTypeCNAME,
			Class: layers.DNSClassIN,
			TTL:   uint32(globalTTLC),
			CNAME: []byte("www.nodnssec.test"),
		},
	},
	"www.nodnssec.test": {
		{
			Name:  []byte("www.nodnssec.test"),
...
var baseAuthoritySection = []layers.DNSResourceRecord{
	{
		Name:  []byte("nodnssec.test"),
		Type:  layers.DNSTypeNS,
		Class: layers.DNSClassIN,
...
var baseAdditionalSection = []layers.DNSResourceRecord{
	{
...
var (
	...
	signerNameC = "nodnssec.test"
	signaturesC = map[string]string{
		"www.nodnssec.test": "YsPPs2WpuC0h7+eYanDr3NZ+boDSkUAUHjhKVGzMlHpkpyjTMpJYsHk/M9Hm2isq5loXKpCS43ILkb/9+kBUpyjS+kMQwN+V7v+4fPECpaA+B+sh1S0E2zT0l24JBbBw",
		"nodnssec.test":     "2g+7jwHGSajuBtOYYM1O+/AC64CdwRaM4lNUryxYdz+Vn/ZqUL/Rxp/RHEFja5a+SwDzME6VwzY47BFLpoQC04afzaxwKSTpn1mw2tRxS+ZLvI/s0XhRLLGbVk0vf9pb",
		"ns1.nodnssec.test": "efiWRospV7iPUMpT6T81FbZ2x10HvEbgS+L/8TvINibNZfi0OLaVm/AQ//tCQjx86Aak8lg2IGH9EmqaeQwdgVG34bLdWbcsin+JvuXOPrB5K5hePxX59LS0svFMWy8P",
...
var (
	...
	signatures = map[string][]byte{
		"www.nodnssec.test": base64Decode(signaturesC["www.nodnssec.test"]),
		"nodnssec.test":     base64Decode(signaturesC["nodnssec.test"]),
		"ns1.nodnssec.test": base64Decode(signaturesC["ns1.nodnssec.test"]),
...
var rrsig = map[string]layers.DNSRRSIG{
	"www.nodnssec.test": {
		...
		Signature:   signatures["www.nodnssec.test"],
	},
	"nodnssec.test": {
		...
		KeyTag:      zskKeyTag,
		SignerName:  signerName,
		Signature:   signatures["nodnssec.test"],
	},
	"ns1.nodnssec.test": {
		TypeCovered: layers.DNSTypeA,
		Algorithm:   layers.DNSSECAlgorithmECDSAP384SHA384,
		...
		KeyTag:      zskKeyTag,
		SignerName:  signerName,
		Signature:   signatures["ns1.nodnssec.test"],
	},
	"ZSK": {
...
func dnsResponseC(dstMAC net.HardwareAddr, dstIP string, dstPort layers.UDPPort, qname string, qtype layers.DNSType, txid uint16) {
	...
	case layers.DNSTypeA:
		if _, ok := answerSection[qname]; !ok {
			// 未配置的域名，返回NXDOMAIN
			dnsLayer = &layers.DNS{
				BaseLayer:    layers.BaseLayer{},
				ID:           txid,
			...
			dnsLayer = &layers.DNS{
				...
				Authorities: baseAuthoritySection,
				Additionals: baseAdditionalSection,
			}
		}
