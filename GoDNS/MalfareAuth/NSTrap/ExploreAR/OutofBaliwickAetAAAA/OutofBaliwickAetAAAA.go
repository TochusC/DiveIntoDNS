 ...
 * @File    :   OutofBaliwickA&AAAA.go
 * @Contact :	tochus@163.com
 * @License :   (C)Copyright 2024
 * @Description: A DNS server that sends out-of-bailiwick A&AAAA records with their RRSIG records in the additional section.
 *
 ...
 * 16/10/24 10:25 	   4stra       0.4.0       Out-of-bailiwick A&AAAA records within AR
 */
...
var additonalSection = func() []layers.DNSResourceRecord {
	...
	for i := 0; i < nsRecordNumC; i++ {
		additional = append(
			additional,
			layers.DNSResourceRecord{
				Name:  []byte(fmt.Sprintf("ww%d.outsider.test", i)),
